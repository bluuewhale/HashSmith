...
public class SwissSimdMap<K, V> extends AbstractArrayMap<K, V> {
    ...
    public void putAll(Map<? extends K, ? extends V> m) {
        ...
        // Pre-check capacity to avoid frequent rehash during batch insertion (align with maybeRehash logic)
        boolean overMaxLoad = (size + tombstones + m.size()) >= maxLoad;

        if (overMaxLoad) {
            // Calculate new capacity to accommodate all elements (rehash will adjust to valid capacity)
            int newSize = this.size + m.size();
            double effectiveLoadFactor = (loadFactor <= 0 || loadFactor > 1) ? DEFAULT_LOAD_FACTOR : loadFactor;
            int minRequiredCap = (int) Math.ceil((double) newSize / effectiveLoadFactor);
            int newCapacity = Math.max(Math.max(capacity * 2, DEFAULT_GROUP_SIZE), minRequiredCap);
            rehash(newCapacity);
        }

        // Batch insert entries without rehash check on each put
